...
datafile= './datasets/air_data.csv' #航空原始数据,第一行为属性标签
...
#读取数据并进行聚类分析
#调用k-means算法，进行聚类分析
kmodel = sklearn.cluster.KMeans(n_clusters = k, n_jobs = 4) #n_jobs是并行数，一般等于CPU数较好
kmodel.fit(data4) #训练模型
...
clu = kmodel.cluster_centers_#找出聚类中心
x = [1,2,3,4,5]
colors = ['red','green','yellow','blue','black']
for i in range(5):
 plt.plot(x,clu[i],label='clustre '+str(i+1),linewidth=6-i,color=colors[i],marker='o')
